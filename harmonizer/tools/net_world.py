...
from ..utils import TransformerAutoencoder
...
class WorldNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.critic_head = create_mlp(
            input_dim = latent_dim + 4, # 4 is the dimension of the output of the time embed.
    ...
    def forward(self, input, tau = 1.0):
        ...
        chord = input[:, 0]
        time = self.time_embed(input[:, 1].unsqueeze(1))
        melody = input[:, 2]
        ...
        memory, lstm_delta = self.lstm(latent)

        ae_chord_delta = oh_chord - chord_recon
        ae_melody_delta = oh_melody - melody_recon

        actor_input = torch.cat((latent, memory), dim = 1)
